...
def evaluate_with_negative_sampling(model, test_df, train_df, num_items, K_values=[1, 3, 5, 10], num_neg=99, seed=42):
    ...
    # Get all items seen in training or test (per user)
    user_seen_items = {}
    ...
    def idcg_at_k(k):
        return 1.0  # Always 1.0 for single-positive items
    ...
    num_users = len(users_in_test)
    if num_users > 0:
        for k in K_values:
            hr_acc[k] /= num_users
            ndcg_acc[k] /= num_users
...
if __name__ == "__main__":
    ...
    user_id_map = {str_id: i + 1 for i, str_id in enumerate(sorted(set(train_df["user_id"].astype(str))))}
    item_id_map = {str_id: i + 1 for i, str_id in enumerate(sorted(set(train_df["item_id"].astype(str)) | set(test_df["item_id"].astype(str))))}
    ...
    item_model = ItemModel(num_items)
    ...
    hr, ndcg = evaluate_with_negative_sampling(model, test_df, train_df, num_items)
